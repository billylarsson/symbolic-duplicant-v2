...
class SQLite:
    def __init__(self,
                 INI_FILENAME, INI_FILE_DIR, DATABASE_FILENAME, DATABASE_FOLDER, DATABASE_SUBFOLDER, ini_variable,
        ):
        """
        this is trying to get all sqlite reads and writes into a single
        class, columns are stored in self.techdict
        ...
        self.ini_variable      = ini_variable
        self.path_to_database  = False
        # ------------------------------------ #
        self.techdict = {}
        self.dev_mode = False
        # ------------------------------------ #
    ...
    def init_connection_and_cursor(self):
        ...
        def ini_file_creation(self, force=False):
            ...
            if not os.path.exists(self.INI_FULL_PATH) or force:
                ...
                if os.path.exists(self.INI_FULL_PATH):
                    with open(self.INI_FULL_PATH, 'r') as f:
                        ...
                        for i in tmp:
                            ...
                            if len(ii) >= 2:
                                varkey = ii[0].strip()
                                keyval = ii[1].strip().rstrip('\n')
                                if varkey and keyval:
                                    data[varkey] = f'{varkey} = {keyval}'
                ...
                with open(self.INI_FULL_PATH, 'w') as f:
                    ...
                    data[self.ini_variable] = f'{self.ini_variable} = "{full_db_path}"'
        ...
        def load_database(self):
            """
            iter each row from INI_FILE until it find both: self.ini_variable
            ...
            with open(self.INI_FULL_PATH, 'r') as f:
                ...
                for row in database_location:
                    ...
                    if self.ini_variable not in row:
                        continue

                    if row[0:len(self.ini_variable)] != self.ini_variable:
                        continue
                    ...
                    self.engine = create_engine('sqlite:///' + loc.full_path,
                        connect_args=dict(check_same_thread=False), poolclass=SingletonThreadPool)
                    ...
                    try:
                        ...
                        self.path_to_database = loc.full_path
                        return True
    ...
    def get_all_tables_and_columns(self):
        ...
        tables = [x for x in tables if x != 'sqlite_sequence']
        columns = [insp.get_columns(x) for x in tables]
        for count, table in enumerate(tables):
            rvdict.update({tables[count]: [x['name'] for x in columns[count]]})
        return rvdict

    def sqlite_superfunction(self, connection, table, column, type, auto):
        """
        ...
        if table not in tables:
            query_create_new_table = 'create table ' + table + auto
    ...
    def db_sqlite(self, table, column, type='text', auto=' (id INTEGER PRIMARY KEY AUTOINCREMENT)'):
        ...
        return self.sqlite_superfunction(self.engine, table, column, type, auto or "")
